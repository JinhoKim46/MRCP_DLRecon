...
Code based on https://github.com/byaman14/ZS-SSL-PyTorch.git
...
def activation_func(activation, is_inplace=False):
    return nn.ModuleDict([["ReLU", nn.ReLU(inplace=is_inplace)], ["None", nn.Identity()]])[activation]
...
class ResNetBlocksModule(nn.Module):
    ...
    def forward(self, x):
        scale_factor = torch.tensor([0.1], dtype=torch.float32).to(x)
...
class ResNet(nn.Module):
    ...
    def __init__(self, in_ch=2, chans=64, num_of_resblocks=5):
        super().__init__()
        self.in_ch = in_ch
        features = chans
        ...
        self.layer2 = ResNetBlocksModule(filter_size=filter2, num_blocks=num_of_resblocks)
